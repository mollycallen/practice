...
class DrumKit {
  ...
  mute(ev) {
    console.log(ev);
    const muteIndex = ev.target.getAttribute("data-track");
...
drumkit.tempoSlider.addEventListener("input", function (ev) {
  drumkit.changeTempoText(ev);
});

drumkit.tempoSlider.addEventListener("change", function (ev) {
  drumkit.updateTempo(ev);
});
